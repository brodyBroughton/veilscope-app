...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[var(--bg)] px-4 py-10">
      <section className="w-full max-w-md rounded-2xl border border-[var(--ui)] bg-[var(--panel)] p-6 text-[var(--ink)] shadow-[0_16px_60px_rgba(2,8,23,.15)]">
        {/* Logo */}
        ...
        <form onSubmit={handleCredentials} className="grid gap-3">
          ...
          {error && (
            <p className="mt-1 rounded-md bg-red-500/10 px-2 py-1 text-sm text-red-600">
              {error}
            </p>
          )}
          ...
          <button
            type="submit"
            disabled={pending}
            className="mt-1 inline-flex h-10 w-full items-center justify-center rounded-xl bg-[var(--accent)] px-4 font-bold text-white disabled:opacity-60"
          >
